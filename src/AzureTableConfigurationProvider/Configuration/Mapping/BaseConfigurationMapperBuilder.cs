...
internal abstract class BaseConfigurationMapperBuilder<TEntity> : IConfigurationMapper<TEntity>, IConfigurationMapperBuilder<TEntity> where TEntity : class
{
    private readonly MappingSection<TEntity> _section;

    protected BaseConfigurationMapperBuilder() => _section = new MappingRootSection<TEntity>();

    private protected BaseConfigurationMapperBuilder(MappingKeySection<TEntity> section) => _section = section;
    ...
    private BaseConfigurationMapperBuilder<TEntity> AddSection(MappingKeySection<TEntity> section, Action<IConfigurationMapper<TEntity>> configure, string sectionName)
    {
        var builder = CreateBuilder(section);
        configure(builder);
    ...
    protected abstract BaseConfigurationMapperBuilder<TEntity> CreateBuilder(MappingKeySection<TEntity> section);

    private BaseConfigurationMapperBuilder<TEntity> WithValueInternal<TValue>(Expression<Func<TEntity, TValue>> valueExpression, string? name)
    {
